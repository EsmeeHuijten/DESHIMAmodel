...
class SubplotAnimationTime(animation.TimedAnimation):
    def __init__(self, F_vector, eta_atm, Pkid, Tb_sky, pwv_vector):
        ...
        self.ax1.legend(['pwv = 0.1 mm', 'pwv = 0.7 mm', 'pwv = 1.0 mm', 'pwv = 1.7 mm'], loc='lower left')
    ...
    def _draw_frame(self, framedata):
        ...
        f_load = np.load(r'E:\data\Esmee\GitHub\DESHIMAmodel\Data\splines_Tb_sky\spline_' \
        + '%.1f' % (self.F_vector[i]) +'GHz.npy', allow_pickle = True)
        # print('f_load', f_load)
        f_function = f_load.item()
        temperature_tot = np.array([])
        power_tot = np.array([])
        errorbars = np.array([len(EL_index), 25])
        for el in range(0, len(EL_index)):
            # el = EL_index[j]
        ...
        plt.savefig(r"E:\data\Esmee\GitHub\DESHIMAmodel\Data\Animations\frames\frame" + str(i))
        self.bar.next()
